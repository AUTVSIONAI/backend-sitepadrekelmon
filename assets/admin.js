...
if(!localStorage.getItem('ADMIN_TOKEN'))localStorage.setItem('ADMIN_TOKEN','padre-admin-local')
...
const saveTexts=async()=>{try{const api=localStorage.getItem('API_BASE')||API_BASE;for(const f of fields){const el=document.querySelector(f.el);if(!el)continue;const val=el.value.trim();let r=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:f.key,value:val})});if(r.status===401){localStorage.setItem('ADMIN_TOKEN','padre-admin-local');r=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:f.key,value:val})})}if(!r.ok)throw new Error()}}catch{return false}return true}
fillTexts()
...
heroSave&&heroSave.addEventListener('click',async()=>{heroMsg.textContent='Salvando...';try{const api=localStorage.getItem('API_BASE')||API_BASE;let url=null;if(heroFile&&heroFile.files&&heroFile.files[0]){const d=await fileToDataURL(heroFile.files[0]);const rm=await fetch(`${api}/api/admin/media`,{method:'POST',headers:h(),body:JSON.stringify({type:'image',title:'hero',alt_text:'hero',url:d,category:'hero'})});if(!rm.ok)throw new Error('media');const m=await rm.json();url=m.url||d}else{const bg=heroPreview.style.backgroundImage||'';url=bg.replace(/^url\(['"]?(.+?)['"]?\)$/,'$1')||null}
  ...
  let r=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:'hero_photo_url',value:{url}})});if(r.status===401){localStorage.setItem('ADMIN_TOKEN','padre-admin-local');r=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:'hero_photo_url',value:{url}})})}
  if(!r.ok)throw new Error('settings-url');
  let r2=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:'hero_photo_style',value:style})});if(r2.status===401){localStorage.setItem('ADMIN_TOKEN','padre-admin-local');r2=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:'hero_photo_style',value:style})})}
...
if(ytSave){ytSave.addEventListener('click',async()=>{const url=$('#youtube-url').value.trim();if(!url){ytStatus.textContent='Informe a URL.';return}try{const api=localStorage.getItem('API_BASE')||API_BASE;let r=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:'youtube_url',value:url})});if(r.status===401){localStorage.setItem('ADMIN_TOKEN','padre-admin-local');r=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:'youtube_url',value:url})})}if(!r.ok)throw new Error();ytStatus.textContent='Vídeo salvo.'}catch{ytStatus.textContent='Falha ao salvar vídeo.'}})}
/* Galeria */
...
galAdd&&galAdd.addEventListener('click',async()=>{const files=$('#gal-photo').files;const category=$('#gal-cat').value;let position=parseInt($('#gal-pos').value||'0',10);if(!files||!files.length){galStatus.textContent='Selecione as fotos.';return}try{const api=localStorage.getItem('API_BASE')||API_BASE;for(const f of files){const d=await fileToDataURL(f);const rm=await fetch(`${api}/api/admin/media`,{method:'POST',headers:h(),body:JSON.stringify({type:'image',title:'galeria',alt_text:'galeria',url:d,category})});if(!rm.ok)throw new Error();const m=await rm.json();const ri=await fetch(`${api}/api/admin/gallery-items`,{method:'POST',headers:h(),body:JSON.stringify({media_id:m.id,category,position,published:true})});if(!ri.ok)throw new Error();position++}galStatus.textContent='Fotos adicionadas.';loadGallery()}catch{galStatus.textContent='Falha ao adicionar.'}})
loadGallery()
/* Chat prompt/model */
const chatSave=$('#chat-save')
const chatStatus=$('#chat-status')
if(chatSave){chatSave.addEventListener('click',async()=>{const model=$('#llm-model').value.trim();const prompt=$('#chat-prompt').value.trim();if(!model||!prompt){chatStatus.textContent='Informe modelo e prompt.';return}try{const api=localStorage.getItem('API_BASE')||API_BASE;let r1=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:'llm_model',value:model})});if(r1.status===401){localStorage.setItem('ADMIN_TOKEN','padre-admin-local');r1=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:'llm_model',value:model})})}let r2=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:'chat_system_prompt',value:prompt})});if(r2.status===401){localStorage.setItem('ADMIN_TOKEN','padre-admin-local');r2=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:'chat_system_prompt',value:prompt})})}if(r1.ok&&r2.ok){chatStatus.textContent='Configurações salvas.'}else{throw new Error()} }catch{chatStatus.textContent='Falha ao salvar.'}})}
const prefill=async()=>{const data=await apiGetSettings();const map=new Map(data.map(i=>[i.key||i.id,i.value]));const m=map.get('llm_model');const p=map.get('chat_system_prompt');const l=map.get('llm_free_models');const y=map.get('youtube_url');if(m)$('#llm-model').value=typeof m==='string'?m:(m.model||'');if(p)$('#chat-prompt').value=typeof p==='string'?p:(p.prompt||'');if(l)$('#llm-free-list').value=typeof l==='string'?l:(Array.isArray(l)?l.join(', '):'');if(y){const val=typeof y==='string'?y:(y.url||'');$('#youtube-url').value=val}};prefill().then(()=>{updateYtPreview()})
...
/* LLM Free Models */
if(llmFreeSave){llmFreeSave.addEventListener('click',async()=>{const list=$('#llm-free-list').value.trim();if(!list){llmFreeStatus.textContent='Informe ao menos um modelo.';return}try{const api=localStorage.getItem('API_BASE')||API_BASE;let r=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:'llm_free_models',value:list})});if(r.status===401){localStorage.setItem('ADMIN_TOKEN','padre-admin-local');r=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:'llm_free_models',value:list})})}if(!r.ok)throw new Error();llmFreeStatus.textContent='Lista de modelos salva.'}catch{llmFreeStatus.textContent='Falha ao salvar a lista.'}})}
