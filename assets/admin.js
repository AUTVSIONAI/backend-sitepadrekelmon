...
if(ytSave){ytSave.addEventListener('click',async()=>{if(!ytArr.length){ytStatus.textContent='Adicione pelo menos um vídeo.';return}try{const api=localStorage.getItem('API_BASE')||API_BASE;let r=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:'youtube_urls',value:ytArr})});if(r.status===401){localStorage.setItem('ADMIN_TOKEN','padre-admin-local');r=await fetch(`${api}/api/admin/settings`,{method:'PUT',headers:h(),body:JSON.stringify({key:'youtube_urls',value:ytArr})})}if(!r.ok)throw new Error();ytStatus.textContent='Lista de vídeos salva.'}catch{ytStatus.textContent='Falha ao salvar vídeos.'}})}
/* Galeria */
